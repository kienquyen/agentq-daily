"""
Experiment v3f — Sector-Relative Foreign Flow
...
Pipeline:
  ...
  Step 3 — Train LightGBM v3f (v3b base + 3 interaction features, TRAIN_START=2020-01-01)  (~3-5 phút)
  Step 4 — Compare AUC + feature importance v3a / v3b / v3f

Chạy:
    python -m pipeline.experiment_v3f_sector_flow
    python -m pipeline.experiment_v3f_sector_flow --step 2   # skip rebuild features
    python -m pipeline.experiment_v3f_sector_flow --step 3   # skip to train only
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(LOG_DIR / "experiment_v3f_sector_flow.log", mode="w"),
    ],
)
...
# New v3f sector-relative features
NEW_INTERACTION_FEATURES = [
    "foreign_sector_share_20d",
    "foreign_sector_rank",
]
...
def step1_rebuild_features() -> None:
    log.info("=" * 60)
    log.info("STEP 1: Rebuild feature files 2022→ với sector-relative features")
    log.info("  New features (%d): %s", len(NEW_INTERACTION_FEATURES), NEW_INTERACTION_FEATURES)
    log.info("  ETA: ~15-20 phút (only 2022→, sector features computed at day level)")
    log.info("=" * 60)
...
def step3_train_v3f() -> dict:
    log.info("=" * 60)
    log.info("STEP 3: Train LightGBM v3f  (~3-5 phút)")
    ...
    # Save to v3f/
    v3f_dir = MODELS_DIR / "v3f"
    v3f_dir.mkdir(parents=True, exist_ok=True)

    pkl_path = v3f_dir / "lgbm_model_20d.pkl"
    with open(pkl_path, "wb") as f:
        pickle.dump({
            ...
            "ret_threshold": 0.05,
            "prob_cutoff":   0.50,
            "experiment":    "interaction_features_v3f",
        }, f)

    metrics_path = v3f_dir / "eval_metrics_20d.json"
    ...
    fi.to_csv(v3f_dir / "feature_importance_20d.csv", index=False)

    log.info("  v3f saved → %s", v3f_dir)
    log.info("Step 3 DONE ✅")
    return {"model": model, "feat_cols": feat_cols, "metrics": metrics, "fi": fi}


# ══════════════════════════════════════════════════════════════════════════════
# Step 4: Compare v3a / v3b / v3f
# ══════════════════════════════════════════════════════════════════════════════

def step4_compare(v3f_result: dict) -> dict:
    log.info("=" * 60)
    log.info("STEP 4: Compare v3a / v3b / v3f + feature importance")
    log.info("=" * 60)
    ...
    v3f_test = v3f_m.get("test", {}).get("roc_auc", 0.0)
    v3f_val  = v3f_m.get("val",  {}).get("roc_auc", 0.0)

    delta_vs_v3a = v3f_test - v3a_test
    delta_vs_v3b = v3f_test - v3b_test

    log.info("")
    log.info("  ┌──────────────────────────────────────────────────────────────────┐")
    log.info("  │          v3a (prod)    v3b (shadow)    v3f (+interaction)      │")
    log.info("  ├──────────────────────────────────────────────────────────────────┤")
    log.info("  │  Test AUC:  %.4f       %.4f          %.4f                │",
             v3a_test, v3b_test, v3f_test)
    log.info("  │  Val  AUC:  %.4f       %.4f          %.4f                │",
             v3a_val,  v3b_val,  v3f_val)
    ...
    log.info("")
    log.info("  New interaction feature importance trong v3f:")
    ...
    log.info("")
    log.info("  All foreign features importance trong v3f:")
    ...
    if delta_vs_v3b >= 0.002:
        verdict = "✅ v3f TỐT HƠN v3b — interaction features cải thiện signal quality"
    elif delta_vs_v3b >= -0.002:
        verdict = "🟡 v3f TƯƠNG ĐƯƠNG v3b — xem feature importance để quyết định"
    else:
        verdict = "⚠️ v3f KÉM HƠN v3b — interaction features chưa đóng góp thêm"

    log.info("")
    log.info("  Kết luận: %s", verdict)
    log.info("  Δ v3f vs v3b: %+.4f  |  Δ v3f vs v3a: %+.4f", delta_vs_v3b, delta_vs_v3a)

    # Save config (inherit from v3b)
    v3f_dir = MODELS_DIR / "v3f"
    try:
        v3b_cfg = load_config("v3b")
        v3f_cfg = v3b_cfg.copy()
    except Exception:
        v3f_cfg = {}

    v3f_cfg.update({
        "_version":      "v3f",
        "_description":  "v3f — foreign flow interaction features (alignment + decel + consistency)",
        "_created":      datetime.now().strftime("%Y-%m-%d"),
        "_experiment":   "interaction_features_v3f",
        "_base_version": "v3b",
        "_new_features": NEW_INTERACTION_FEATURES,
        "_auc_v3a":      round(v3a_test, 4),
        "_auc_v3b":      round(v3b_test, 4),
        "_auc_v3f":      round(v3f_test, 4),
        "_auc_delta_vs_v3b": round(delta_vs_v3b, 4),
        "_auc_delta_vs_v3a": round(delta_vs_v3a, 4),
    })
    (v3f_dir / "config.json").write_text(
        json.dumps(v3f_cfg, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    log.info("  Config saved → %s", v3f_dir / "config.json")

    return {
        "v3a_test": v3a_test, "v3b_test": v3b_test, "v3f_test": v3f_test,
...
def main(start_step: int = 1) -> None:
    t_total = time.time()
    log.info("╔══════════════════════════════════════════════════════╗")
    log.info("║  Experiment v3f — Sector-Relative Foreign Flow  ║")
    log.info("╚══════════════════════════════════════════════════════╝")
    ...
    if start_step <= 3:
        result3 = step3_train_v3f()
    else:
        log.info("STEP 3: SKIPPED — loading existing v3f")
        import pandas as pd
        v3f_dir = MODELS_DIR / "v3f"
        with open(v3f_dir / "lgbm_model_20d.pkl", "rb") as f:
            payload = pickle.load(f)
        with open(v3f_dir / "eval_metrics_20d.json") as f:
            metrics = json.load(f)
        fi = pd.read_csv(v3f_dir / "feature_importance_20d.csv")
        result3 = {
            "model":     payload["model"],
    ...
    log.info("")
    log.info("╔══════════════════════════════════════════════════════╗")
    log.info("║  EXPERIMENT v3f COMPLETE  (%.0f phút)               ║", elapsed / 60)
    log.info("║  v3a: %.4f → v3b: %.4f → v3f: %.4f          ║",
             comparison["v3a_test"], comparison["v3b_test"], comparison["v3f_test"])
    log.info("║  %s", comparison["verdict"])
    log.info("╚══════════════════════════════════════════════════════╝")
    log.info("")
    log.info("Bước tiếp theo nếu v3f tốt hơn hoặc tương đương v3b:")
    log.info("  1. Chạy backtest_compare để xem WR và mean_ret thực tế")
    log.info("  2. python -m pipeline.backtest_compare (v3f sẽ tự detect nếu có pkl)")
    log.info("  3. Nếu backtest tốt hơn v3b → replace shadow: echo v3f > data/models/shadow_version.txt")
