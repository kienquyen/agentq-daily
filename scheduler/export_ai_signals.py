...
def _trading_calendar() -> list[str]:
    """Lấy danh sách ngày giao dịch từ feature parquet files."""
    feat_dir = CURR / "data" / "features"
    dates = sorted(p.stem for p in feat_dir.glob("20*.parquet"))
    return dates

def _add_n_trading_days(date_str: str, n: int, calendar: list[str]) -> str | None:
    """Trả về ngày giao dịch thứ n sau date_str, dựa vào calendar."""
    future = [d for d in calendar if d > date_str]
    if len(future) >= n:
        return future[n - 1]
    return None  # chưa đủ n phiên (signal còn pending)
...
def export_v5():
    ...
    # Trading calendar để tính T+20 exit date
    t_cal = _trading_calendar()
                ...
                for s in d.get("signals", []):
                    ...
                    if not df_log.empty:
                        ...
                        if not match.empty:
                            ...
                            # Không có trong log → tính T+20 exit_date từ trading calendar
                            t20 = _add_n_trading_days(date_str, 20, t_cal)
                            today_str = datetime.utcnow().strftime("%Y-%m-%d")
                            if t20 and t20 <= today_str:
                                status    = "completed"
                                exit_date = t20
                            else:
                                status = "pending"
